...
import { OptionChain, StrikeMap, ExpirationDateMap } from "tda-api";
...
export function format_text(result: FormatInput) {
  ...
  let output = _.chain(result.results)
    .map((r) => {
      ...
      return `${type} $${_.padEnd(r.strikePrice.toString(), 5, ' ')} ${r.expiration} D:${Math.round(Math.abs(r.delta) * 100)} for ${r.targetDelta * 100} V:${r.totalVolume} OI:${r.openInterest} BA:${r.bid}-${r.ask} (${r.spreadPercent.toFixed(1)}%)`;
    })
    .compact()
    .value();
  ...
  output.unshift(`Current: ${un.last} (${sign}${un.change}  ${un.percentChange.toFixed(2)}%) H:${un.highPrice} L:${un.lowPrice}`);
