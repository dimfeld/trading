...
import * as tda_api from 'tda-api';
import { DbData, IPosition, IStrategies } from '../types';
import { check_expired_legs } from './expired_legs';
import { write_positions } from '../db';
import { print_trade_description, describe_position, format_money, describe_sorted_legs, describe_leg, sort_legs_for_display } from '../ui';
...
export async function check_active(data : DbData, args : Args) {
  let position_chain = _.chain(data.positions)
    .flatMap((x) => x);
  let hasSymbols = args.symbol && args.symbol.length;
  if(hasSymbols) {
    position_chain = position_chain.filter((p) => _.includes(args.symbol, p.symbol));
  }

  if(args.strategy) {
    position_chain = position_chain.filter((p : IPosition) => {
  ...
  if(args.check_expired) {
    ...
    if(updated_positions.length) {
      ...
      await write_positions(updated_positions.map((up) => up.position), newTrades);
...
function leg_list(args : Args, positions : IPosition[]) {
  let symbols = new Set<string>();

  _.each(positions, (p) => {
    _.each(p.legs, (leg) => {
      if(!args.symbol.length || _.find(args.symbol, (allowed) => leg.symbol.startsWith(allowed))) {
...
async function leg_prices(leg_set : Set<string>) {
  ...
  let api = new tda_api.Api(auth);
  await api.init();

  let results : _.Dictionary<tda_api.Quote> = {};
...
function cost_to_close(quotes : _.Dictionary<tda_api.Quote>, position : IPosition) {
  let total_current = 0;
  let total_change = 0;
  ...
  _.each(position.legs, (leg) => {
    ...
    let multiplier = quote.assetType === tda_api.AssetType.Option ? 100 : 1;
    let total_size = multiplier * leg.size;
    total_current += total_size * quote.mark;
    total_change += total_size * quote.netChange;
    debug("Symbol %s size %d mark %d change %d", leg.symbol, total_size, quote.mark, quote.netChange);
  });
...
function print_totals({ cost_basis, closing_cost, realized, total_pnl }) {
  ...
  console.log(`Basis: ${basis}, Cost To Close: ${format_money(closing_cost.current, false)}, Total PnL: ${format_money(total_pnl)} ${pnl_pct_fmt}%, Unrealized Change: ${format_money(closing_cost.change)}`);
}

function show_position(strategies : IStrategies, quotes : _.Dictionary<tda_api.Quote>, position : IPosition) {
  ...
  let leg_desc = _.map(sorted_legs, (leg) => {
    ...
    let pct_change = format_money(quote.netChange / quote.closePrice, true, {prefix: '', suffix: '%'});
    return `${desc} ${format_money(quote.mark, false)}  ${format_money(quote.netChange)} ${pct_change}`;
  });
