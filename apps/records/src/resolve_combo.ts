...
import { DbOptionLeg, DbPosition, DbData } from 'types';
import { oneline_position_detail } from './ui';

function parse_leg(underlying: string, desc: string): DbOptionLeg {
...
export async function resolve_combo(
  symbol,
  size,
  price,
  date,
  ...
  let description = `the ${size} combo${
    size > 1 ? 's' : ''
  } for \$${price} in ${symbol} on ${date}`;

  let existing = db_data.positions[symbol] || [];
  let candidate_positions = _.filter(existing, (p) =>
    Boolean(p.legs && p.legs.length)
  );
  ...
  let data = await inquirer.prompt<Combo>([
    {
      ...
      validate: (x) =>
        +x >= 0 && +x <= 4 ? true : 'must be a number from 0 through 4',
    },
    {
      type: 'input',
      name: 'leg1',
  ...
  if (legs.length === 0) {
    legs = [parse_leg(symbol, '1/1 50 call'), parse_leg(symbol, '1/1 50 put')];
  }
